...
Note: During the start of a live deployment startup calibration runs the model over a recent or full cached feature frame to score how effective trades look in each regime (trend vs mean reversion), takes a percentile of those scores to set fresh entry cutoffs, clips them, checks sample counts, and, if valid, applies the new cutoffs; it logs batch progress, regime stats, and can write the resulting thresholds to startup_thresholds.json, so the live gate is tuned to current data before orders go out.
